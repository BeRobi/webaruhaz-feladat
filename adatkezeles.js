...
export function tablazatOsszeallit(lista, hova) {
  ...
  if (szamlalo == 0) {
    ...
    for (const KULCS in kutyaKULCS)
      elem_letrehozo(tablasorElem, "th", "aria-sort").appendChild(document.createTextNode(kutyaKULCS[KULCS]));
  ...
  for (const KULCS in lista) {
      elem_letrehozo(tablasorElem, "td", "cella").appendChild(document.createTextNode(lista[KULCS]));
  }
